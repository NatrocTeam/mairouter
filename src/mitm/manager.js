...
const DEFAULT_MITM_ROUTER_BASE = "http://localhost:12890";
...
// (prevents EBUSY on `npm install -g mairouter@latest` while MITM is running).
...
const ENCRYPT_SALT = "mairouter-mitm-pwd";
...
async function startServer(apiKey, sudoPassword, forceKillPort443 = false) {
  ...
  try {
    ...
    if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
      ...
      if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
        throw new Error(
          `MITM server.js not found at ${effectiveServerPath}. Reinstall mairouter.`,
        );
      }
    }

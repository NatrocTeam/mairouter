...
const hasmairouterConfig = (config) => {
  ...
  return !!config.provider["mairouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["mairouter"];
    ...
    return NextResponse.json({
      ...
      opencode: {
        ...
        activeModel: config?.model?.startsWith("mairouter/")
          ? config.model.replace(/^mairouter\//, "")
          : null,
...
// POST - Apply mairouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    // Preserve any existing mairouter provider entry and its models
    const existingProvider = config.provider["mairouter"] || {
    ...
    config.provider["mairouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `mairouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `mairouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("mairouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove mairouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["mairouter"]?.models) {
      delete config.provider["mairouter"].models[modelToRemove];

      // If no models left, remove the provider
      if (Object.keys(config.provider["mairouter"].models).length === 0) {
        delete config.provider["mairouter"];
        if (config.model?.startsWith("mairouter/")) delete config.model;
      } else if (config.model === `mairouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(
          config.provider["mairouter"].models,
        );
        config.model = `mairouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire mairouter provider
      if (config.provider) delete config.provider["mairouter"];
      if (config.model?.startsWith("mairouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("mairouter/")) {
    ...
    return NextResponse.json({
      ...
      message: modelToRemove
        ...
        : "mairouter settings removed from OpenCode",
    });
  } catch (error) {

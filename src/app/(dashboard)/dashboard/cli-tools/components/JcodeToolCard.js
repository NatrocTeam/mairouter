...
export default function JcodeToolCard({
  ...
  const getConfigStatus = () => {
    ...
    if (!jcodeStatus?.hasmairouter) return "not_configured";
    const currentProvider = jcodeStatus.config?.providers?.["mairouter"];
  ...
  const handleApplySettings = async () => {
    ...
    try {
      const keyToUse =
        ...
        (!cloudEnabled ? "sk_mairouter" : null);
  ...
  const getManualConfigs = () => {
    ...
    const configToml = `[providers.mairouter]
...
[[providers.mairouter.models]]
id = "${selectedModel || "cc/claude-opus-4-7"}"`;

    const envContent = `JCODE_mairouter_API_KEY="${keyToUse}"`;
    ...
    return [
      ...
      {
        filename: "~/.config/jcode/provider-mairouter.env",
        content: envContent,
      },
    ];
  };
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted mt-2">
                      Manual configuration is still available if mairouter is
          ...
          {!checkingJcode && jcodeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {jcodeStatus?.config?.providers?.["mairouter"]?.base_url && (
                ...
                <div className="flex flex-col gap-1 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                  ...
                  <code className="text-xs font-mono text-text-muted">
                    jcode --provider-profile mairouter
                  </code>
                  <code className="text-xs font-mono text-text-muted">
                    jcode --provider-profile mairouter --model{" "}
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button
                  ...
                  disabled={!jcodeStatus?.hasmairouter}
                  loading={restoring}
                >
                  <span className="material-symbols-outlined text-[14px] mr-1">
                    restore

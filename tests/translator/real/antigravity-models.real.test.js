// REAL integration test: hits localhost:12890/v1 with real API key for all antigravity models.
...
//   RUN_REAL=1 AG_URL=http://localhost:12890/v1 AG_KEY=sk-xxx npx vitest run ...
//
import { describe, it, expect } from "vitest";

const RUN_REAL = process.env.RUN_REAL === "1";
const BASE_URL = process.env.AG_URL || "http://localhost:12890/v1";
  ...
  for (const model of AG_MODELS) {
    it(
      ...
      async () => {
        ...
        if (status === 400) {
          console.warn(
            `[skip] ${model}: 400 (provider quirk): ${text.slice(0, 200)}`,
          );
          return;
        }
        ...
        expect(
          hasContent,
          `${model} simple: no content in ${text.slice(0, 300)}`,
        ).toBeTruthy();
      },
      TIMEOUT_MS,
    );
      ...
      async () => {
        ...
        if (status === 400) {
          ...
          if (
            ...
            throw new Error(
              `BUG: ${model} — 400 due to optional field: ${errMsg.slice(0, 300)}`,
            );
          }
          // Other 400 (e.g. model doesn't support tools, stream_options issue) → skip
          console.warn(
            `[skip] ${model}: 400 non-optional error: ${errMsg.slice(0, 200)}`,
          );
          return;
        }

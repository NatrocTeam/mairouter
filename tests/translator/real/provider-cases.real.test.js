...
describe.skipIf(!RUN_REAL)("REAL provider behavior cases", () => {
  ...
  it(
    ...
    async () => {
      ...
      expect(
        /"finish_reason"\s*:\s*"stop"/.test(out.raw),
        "no stop finish_reason",
      ).toBe(true);
    },
    TIMEOUT_MS,
  );
  ...
  it(
    ...
    async () => {
      ...
      if (!/"finish_reason"\s*:\s*"length"/.test(out.raw)) {
        console.warn(
          "[skip] ollama: model did not hit length (output shorter than max_tokens)",
        );
  ...
  it(
    ...
    async () => {
      ...
      const messages = [
        ...
        {
          role: "assistant",
          content: "Understood. I have noted that context.",
        },
      ...
      if (!m) {
        console.warn(
          "[skip] codex: no cached_tokens in usage (provider may not report)",
        );
        return expect(true).toBe(true);
      }
      expect(Number(m[1]), "cached_tokens not > 0 on 2nd turn").toBeGreaterThan(
        0,
      );
    },
    TIMEOUT_MS,
  ...
  it(
    ...
    async () => {
      ...
      expect(
        /data:|finish_reason|"delta"|"content"|event:/.test(out.raw),
        "not SSE",
      ).toBe(true);
    },
    TIMEOUT_MS,

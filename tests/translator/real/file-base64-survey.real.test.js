  ...
  if (fmt === "gemini" || fmt === "gemini-cli" || fmt === "antigravity") {
    const gem = {
      contents: [
        {
          role: "user",
          parts: [{ text: ask }, { inlineData: { mimeType: mime, data: b64 } }],
        },
      ],
    ...
    return fmt === "antigravity"
      ? {
          ...
          body: { request: gem, userAgent: "antigravity" },
        }
...
describe.skipIf(!RUN_REAL)("REAL file base64 survey", () => {
    ...
    for (const [kind, mime, b64] of FILE_TYPES) {
      it.concurrent(
        ...
        async () => {
          ...
          const credIssue =
            CRED_ISSUE.includes(status) || CRED_MSG_RE.test(errMsg);
  ...
  afterAll(() => {
    ...
    const rank = (v) =>
      v.startsWith("ok") ? 0 : v.startsWith("reject") ? 1 : 2;

...
describe("OpenAI → Claude context mapping", () => {
  ...
  it("assistant reasoning_content is preserved without forging a signed thinking block", () => {
    const out = T({
      messages: [
        { role: "user", content: "q" },
        {
    ...
    expect(assistant.content[0]).toEqual({
      type: "text",
      text: "[Reasoning from previous assistant]\nmy hidden reasoning",
    });
    expect(assistant.content.some((block) => block.type === "thinking")).toBe(
      false,
    );
  });

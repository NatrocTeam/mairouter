...
const resolved = {
  aliasToId: Object.fromEntries(
    ALIAS_TOKENS.map((a) => [a, resolveProviderAlias(a)]),
  ),
...
if (JSON.stringify(baseline) === JSON.stringify(current)) {
  console.log(
    `✅ Alias resolution byte-for-byte equal (${ALIAS_TOKENS.length} tokens).`,
  );
  process.exit(0);
}
// Diff
for (const a of ALIAS_TOKENS) {
  if (baseline.aliasToId[a] !== current.aliasToId[a]) {
    console.error(
      `~ alias ${a}: ${baseline.aliasToId[a]} -> ${current.aliasToId[a]}`,
    );
  }
}

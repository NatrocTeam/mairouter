...
describe("quota auto-ping", () => {
  ...
  beforeEach(async () => {
    ...
    deps = {
      ...
      refreshAndUpdateCredentials: vi.fn(async (connection) => ({
        connection,
        refreshed: false,
      })),
      ...
      getExecutor: vi.fn(() => ({
        execute: vi.fn().mockResolvedValue({
          response: { ok: true, text: codexResponseText },
        }),
      })),
    };
    ...
    getExecutor.mockReturnValue({
      execute: vi
        .fn()
        .mockResolvedValue({ response: { ok: true, text: codexResponseText } }),
    });
    ...
    getCodexUsage.mockResolvedValue({
      quotas: {
        session: {
          ...
          total: 100,
          remaining: 99,
          resetAt: "2026-01-01T17:01:00.000Z",
        },
      },
    });
    ...
    getCodexUsage.mockResolvedValue({
      quotas: {
        session: {
          ...
          total: 100,
          remaining: 100,
          resetAt: "2026-01-01T11:59:47.000Z",
        },
      },
    });
  ...
  it("keeps Claude session quota key behavior", async () => {
    deps.getSettings.mockResolvedValue({
      claudeAutoPing: { connections: { "claude-1": true } },
    });
    deps.getProviderConnections.mockImplementation(async ({ provider }) =>
      provider === "claude"
        ? [
    ...
    expect(
      JSON.parse(deps.proxyAwareFetch.mock.calls[0][1].body),
    ).toMatchObject({

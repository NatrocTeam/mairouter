...
describe("stripUnsupportedModalities", () => {
  ...
  it("openai: strips image when vision:false, leaves placeholder", () => {
    ...
    expect(
      body.messages[0].content.some(
        (b) => b.type === "text" && /image omitted/.test(b.text),
      ),
    ).toBe(true);
  });
  ...
  it("openai: strips input_audio when audioInput:false", () => {
    ...
    expect(body.messages[0].content.some((b) => b.type === "input_audio")).toBe(
      false,
    );
    expect(
      body.messages[0].content.some((b) => /audio omitted/.test(b.text || "")),
    ).toBe(true);
  });
  ...
  it("openai: keeps image when vision:true", () => {
    ...
    stripUnsupportedModalities(body, FORMATS.OPENAI, NO_AUDIO);
    expect(body.messages[0].content.some((b) => b.type === "image_url")).toBe(
      true,
    );
  });
  ...
  it("gemini: strips inlineData image by mime when vision:false", () => {
    ...
    expect(
      body.contents[0].parts.some((p) => /image omitted/.test(p.text || "")),
    ).toBe(true);
  });
  ...
  it("antigravity: strips inside request.contents", () => {
    ...
    expect(body.request.contents[0].parts.some((p) => p.inlineData)).toBe(
      false,
    );
  });
  ...
  it("responses: strips input_image when vision:false", () => {
    ...
    expect(body.input[0].content.some((b) => b.type === "input_image")).toBe(
      false,
    );
    expect(
      body.input[0].content.some(
        (b) => b.type === "input_text" && /image omitted/.test(b.text),
      ),
    ).toBe(true);
  });

  it("handles missing/empty body safely", () => {
    expect(stripUnsupportedModalities(null, FORMATS.OPENAI, NO_VISION)).toBe(
      false,
    );
    expect(stripUnsupportedModalities({}, FORMATS.OPENAI, null)).toBe(false);
    expect(
      stripUnsupportedModalities({ messages: [] }, FORMATS.OPENAI, NO_VISION),
    ).toBe(true);
  });
});

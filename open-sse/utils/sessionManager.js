 ...
 * Since mairouter is long-running, we simulate this "per-launch" behavior by
 * storing a generated ID in memory for each connection.
 *
 * - If mairouter restarts, the ID changes (matching binary restart behavior).
...
function extractAntigravitySession(body) {
  ...
  const m =
    typeof body?.requestId === "string"
      ? body.requestId.match(ANTIGRAVITY_CONV_RE)
      : null;
...
export function resolveSessionId({
  ...
  const fromAssistant = assistantTextSessionId(
    `${scope}:${connectionId || ""}`,
    body,
  );

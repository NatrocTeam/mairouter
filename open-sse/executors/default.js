...
const HEADER_HOOKS = {
  ...
  clineHeaders: (h, c) =>
    Object.assign(h, buildClineHeaders(c.apiKey || c.accessToken)),
  kilocodeOrg: (h, c) => {
...
export class DefaultExecutor extends BaseExecutor {
  ...
  buildHeaders(credentials, stream = true) {
    ...
    const desc =
      rt?.auth ||
      AUTH_DESCRIPTORS[this.provider] ||
      this.resolveAuthDescriptor();
    // Hooks run BEFORE auth so dynamic overlays (claude cached headers) can't clobber the token.
    for (const hook of desc.hooks || [])
      HEADER_HOOKS[hook]?.(headers, credentials);
  ...
  async refreshKimiCoding(refreshToken, proxyOptions = null) {
    ...
    const response = await proxyAwareFetch(
      PROVIDERS["kimi-coding"].refreshUrl,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",

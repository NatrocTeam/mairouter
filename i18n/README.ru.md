...
🎉 Панель управления откроется на `http://localhost:12890`
...
cp .env.example .env
npm install
PORT=12890 NEXT_PUBLIC_BASE_URL=http://localhost:12890 npm run dev
```
...
```bash
npm run build
PORT=12890 HOSTNAME=0.0.0.0 NEXT_PUBLIC_BASE_URL=http://localhost:12890 npm run start
```

URL по умолчанию:

- Панель управления: `http://localhost:12890/dashboard`
- OpenAI-совместимый API: `http://localhost:12890/v1`
...
Settings → Models → Advanced:
  OpenAI API Base URL: http://localhost:12890/v1
...
```json
{
  "anthropic_api_base": "http://localhost:12890/v1",
...
```bash
export OPENAI_BASE_URL="http://localhost:12890"
...
```json
{
  ...
  "models": {
    "providers": {
      "9router": {
        "baseUrl": "http://127.0.0.1:12890/v1",
        "apiKey": "sk_9router",
...
Base URL: http://localhost:12890/v1
...
export NEXT_PUBLIC_BASE_URL="http://localhost:12890"
...
| `PORT`                                               | framework default                       | Порт сервиса (`12890` в примерах)                                                                    |
...
| `BASE_URL`                                           | `http://localhost:12890`                | Внутренний серверный базовый URL для задач облачной синхронизации                                    |
...
- Установите `PORT=12890` и `NEXT_PUBLIC_BASE_URL=http://localhost:12890`

**Ошибки облачной синхронизации**

- Убедитесь, что `BASE_URL` указывает на ваш работающий инстанс (например, `http://localhost:12890`)
...
```bash
POST http://localhost:12890/v1/chat/completions
...
```bash
GET http://localhost:12890/v1/models
Authorization: Bearer your-api-key
...
- Локально (`http://127.0.0.1:12890/v1/chat/completions`): работает с `stream=false` и `stream=true`.
